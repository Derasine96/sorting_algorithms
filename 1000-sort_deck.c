...
void insertion_sort_deck_kind(deck_node_t **deck)
{
	deck_node_t *ptr, *insert, *temp;

	for (ptr = (*deck)->next; ptr != NULL; ptr = temp)
	{
		temp = ptr->next;
		insert = ptr->prev;
		while (insert != NULL && insert->card->kind > ptr->card->kind)
		{
			insert->next = ptr->next;
			if (ptr->next != NULL)
				ptr->next->prev = insert;
			ptr->prev = insert->prev;
			ptr->next = insert;
			if (insert->prev != NULL)
				insert->prev->next = ptr;
			else
				*deck = ptr;
			insert->prev = ptr;
			insert = ptr->prev;
...
void insertion_sort_deck_value(deck_node_t **deck)
{
	...
	for (ptr = (*deck)->next; ptr != NULL; ptr = temp)
	{
		...
		insert = ptr->prev;
		while (insert != NULL &&
				insert->card->kind == ptr->card->kind &&
				get_card_value(insert) > get_card_value(ptr))
		{
			insert->next = ptr->next;
			if (ptr->next != NULL)
				ptr->next->prev = insert;
